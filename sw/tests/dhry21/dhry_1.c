  ...
  /* Main and Proc_0 in the Ada version             */
{
...
#endif
  printf("HZ %u, CPU MHz %u.%03u\n", HZ, CPU_CLK/1000000, (CPU_CLK/1000)%1000);
  ...
  printf("Time: begin= %lu, end= %lu, diff= %lu\n",
         (unsigned long)Begin_Time, (unsigned long)End_Time, (unsigned long)User_Time);
...
#endif
#else
    Microseconds = (User_Time * (Mic_secs_Per_Second / (HZ / 1000))) / Number_Of_Runs;
    Dhrystones_Per_Second = ((unsigned long long)HZ * (unsigned long long)Number_Of_Runs) / User_Time;

  printf("Microseconds for one run through Dhrystone: %ld.%03ld\n", Microseconds / 1000, Microseconds % 1000);
